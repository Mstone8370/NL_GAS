...
{
    return WeaponTag.IsValid() && Data.Contains(WeaponTag) && Data[WeaponTag].PatternSequence != nullptr;
}
...
FVector UWeaponRecoilPattern::GetRecoilPatternAt(const FGameplayTag& WeaponTag, int32 Offset) const
{
    ...
    float MinTime = 0.f;
    float MaxTime = 0.f;
    RecoilInfo.PatternSequence->GetTimeRange(MinTime, MaxTime);
    ...
    if (Offset < PatternLength)
    {
        ...
        return RecoilInfo.PatternSequence->GetVectorValue(CurveTime);
    }

    // In loop
    if (LoopLength < 1)
    {
        return RecoilInfo.PatternSequence->GetVectorValue(OffsetBegin + LoopStartOffset);
    }
